...
//		$this->_send_reply($message);
        ...
        public function process_duty_message($site_id,$shift_type,$message){
            //echo 'call duty message';exit;
           //*************************** Twilio Details Start*******************************//    
                $twilio_det = $this->CI->twilio_model->get_row();
                $this->_session->twilio_number = $twilio_det->twilio_number;
                $this->_session->twilio_sid = $twilio_det->twilio_sid;
                 $this->_session->twilio_token = $twilio_det->twilio_token;
          //*************************** Twilio Details Ends*******************************//  
           $number= $this->CI->duty_model->get_staff_member($site_id,$shift_type);
            //$staff_number = $this->CI->staff_model->getNumber($staff_id);
            var_dump($number[0]->phone_number);exit;
                    
            foreach ($number as $num){
                echo $this->send_sms_message($num->phone_number, urldecode($message));
	...
	{
           //var_dump($this->_session->twilio_sid);exit;
            try{    
$twilio_det = $this->CI->twilio_model->get_row();
                 $this->_session->twilio_number = $twilio_det->twilio_number; 
                $this->_session->twilio_sid = $twilio_det->twilio_sid;
