...
class Work_status extends Site_admin_controller
{	
	...
	{
		parent::__construct();
		$this->load->model(array('work_status_model',
								)
						);
	...
	{	
		$this->_sort_order = $this->work_status_model->get_sort_order();
		...
		$rows = $this->work_status_model->get_rows($this->_filter, $this->_sort_order);
		...
		$this->load->view('admin/work_status_list', $data);		
	}
	...
	{
		if ( ! $this->work_status_model->load_by_id(intval($id)) ) {
			set_message_note($this->lang->line('error_no_record'), MESSAGE_NOTE_FAILURE);
			redirect('admin/work_status');
		}
		...
		$this->form_validation->set_rules('text_color', 'Text Color', '');
		$this->form_validation->set_rules('background_color', 'Background Color', '');
	
		
		
		$this->_row = $this->work_status_model->get_row();
		...
		$this->load->view('admin/work_status_edit', $data);		
	}
	
	protected function _save()
	{
		if ( $this->form_validation->run() ) {
			$this->work_status_model->update_row($this->_row, $this->staff_model->id);
