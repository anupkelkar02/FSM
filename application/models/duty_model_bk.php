...
on gsite.id=gsa.site_id inner join grs_staff staff on staff.id=gsa.staff_id where gs.site_id='".$site_id."' and gs.shift_type='".$shift_type."' and work_status_id in (1,2) and start_date=date_format(now(),'%Y-%m-%d') group by staff.id";  
            //echo $sql;exit;
            $query = $this->db->query($sql);
            return $query->result();
